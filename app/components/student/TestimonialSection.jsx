import { assets, dummyTestimonial, testimonials } from '@/app/assets/assets'
...
const TestimonialSection = () => {
  return (
    <div className='pb-14 px-8 md:px-0'>
      ...
      {/* <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-14 mx-auto px-auto md:px-40'>
        {dummyTestimonial.map((testimonial, index) => (
          <div key={index} className='text-sm text-left border border-gray-500/30 pb-6 rounded-lg bg-white shadow-[0px_4px_15px_0px] shadow-black/5 overflow-hidden'>
            <div className='flex items-center gap-4 px-5 py-4 bg-gray-500/10'>
              <Image className='h-12 w-12 rounded-full' src={testimonial.image} alt={testimonial.name} />
              <div>
            ...
            <div className='p-5 pb-7'>
              <div className='flex gap-0.5'>
                {[...Array(5).map((_, i) => (
                  <Image className='h-5' key={i} src={i < Math.floor(calculateRating(course)) ? assets.star : assets.star_blank} alt='star' />
                ))]}
              </div>
